...
const Dispatched_Items_Site = () => {
  ...
  const OrderDetails = (order) => {
    ...
    if (order.transfer_status === "site_to_site") {
      source = "transfer";
    }

    setDetails({ ...order, source });
    setIsOpen(true);
  };
  ...
  useEffect(() => {
    if (user?.id && allSites.length > 0) {
      fetchSiteTransfers();
    }
  }, [user?.id, allSites]);

  // ✅ filter dispatched items for only those sites
  const siteDispatchedItems = Outwards.filter(
    (order) => siteIds.includes(order.site?.id) && order.type === "dispatched"
  );

  const combinedDispatchedItems = [
    ...siteDispatchedItems.map((item) => ({
      ...item,
      source: "outward",
    })),
    ...siteTransfers.map((item) => ({
      ...item,
      source: "transfer",
    })),
  ];
  ...
  const markAsCompleted = async () => {
    try {
      if (details.source === "outward") {
        await fetch(`${api}/api/dispatch/complete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ dispatch_id: details.id }),
        });
      }

      if (details.source === "transfer") {
        await fetch(`${api}/api/transfer/complete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ transfer_id: details.id }),
        });
      }
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold">
              Dispatched Item list for your List
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {combinedDispatchedItems.map((order) => (
                  <tr
                    key={order.id}
                    ...
                    <td className="py-3 px-4">
                      <Badge
                        className={getStatusColor(
                          order.source === "outward" ? order.type : order.status
                        )}
                      >
                        {order.source === "outward" ? order.type : order.status}
                      </Badge>
...
export default Dispatched_Items_Site;
